...
2. Read existing config: check if `.gatling/package.conf` already exists. Also check `.gatling/example.package.conf` for reference.
...
4. Ask the user what they need with the `AskUserQuestion` tool.
   ...
   - Package name (default to Maven artifactId)
...
## Package Descriptor file example
